...
const darkFieldStyle = { backgroundColor: "#0a1428", colorScheme: "dark" as const };

export function Input(props: InputHTMLAttributes<HTMLInputElement>) {
  return (
    <input
      style={{ ...darkFieldStyle, ...props.style }}
      className={cn(baseFieldStyles, props.className)}
      {...props}
    />
  );
}
...
export function Textarea(props: TextareaHTMLAttributes<HTMLTextAreaElement>) {
  return (
    <textarea
      style={{ ...darkFieldStyle, ...props.style }}
      className={cn(baseFieldStyles, "min-h-[180px] resize-y", props.className)}
      {...props}
    />
  );
}
